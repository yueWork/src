...
public class SubmitOrder extends HttpServlet{
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String bcounter = request.getParameter("bcounter");
		String state = "1";
		...
		String state0="1";
		//order表中插入数据
		String sql = "insert into bookstore.order (uid,oid,state,time) values (\"" + uid + "\",\"" + oid + "\",\"" + state + "\",\"" + time + "\");";
		System.out.println(sql);
		//shop表中插入数据
		String sql1 = "insert into bookstore.shop (oid,bid,counter) values (\"" + oid + "\",\""+ bid + "\",\"" + bcounter + "\");";
		...
		if(state0 == "1"){
			try {
				...
				state0="0";
				System.out.println("order插入失败");
			}
		}
		
		if(state0 == "1"){
			try {
				con_data.pst = con_data.connection.prepareStatement(sql1);// 准备执行SQL语句
				con_data.pst.executeUpdate();
				con_data.pst.close();
			
				System.out.println("shop插入成功！");
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				state0="0";
				System.out.println("shop插入失败");
			}
		}
